...
select first_name, hire_date, salary, floor(months_between(sysdate, hire_date)/12) 근무년차
from employees
...
-- 8. 각 부서별 최대급여와 최소급여를 조회하시오. 단, 최대급여와 최소급여가 같은 부서는 직원이 한명일 가능성이 높기때문에 조회결과에서 제외시킨다.
...
-- 단, 평균급여가 8000 이상인 부서만 출력되어야 하며, 출력결과를 평균급여가 높은 부서면저 출력되도록 해야 한다.
select department_id ,floor(avg(salary)) 평균급여
from employees
